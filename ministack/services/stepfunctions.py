...
# Services whose SDK names each error as its own class, so Step Functions reports
# "<Service>.<Code>Exception" while the wire <Code> has no suffix. S3 is deliberately absent.
_REST_XML_ERROR_SUFFIXED = {"route53"}
    ...
    if status >= 400:
        ...
        if decoded:
            try:
                ...
                pass
        if service_key in _REST_XML_ERROR_SUFFIXED and not code.endswith("Exception"):
            code += "Exception"
